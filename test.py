...
#### testing #####
#loader里面包含
#data.train_loader 训练集数据
#data.test_norm_loader 测试集正常数据
#data.test_anom_loader 测试集异常数据
loader = [data.train_loader,data.test_norm_loader,data.test_anom_loader]

t_loss_norm =[]
t_loss_anom =[]
end = time.time()
print("导模型评估循环运行时间:%.4f秒"%(end-start))
def Thresholding(data_load = loader[1:], upsample = 1, thres_type = 0, fpr_thres = 0.3):
    ...
    normalised_score_t = []
    mask_score_t = []
    #for 循环中的第一个data是normal 第二个data是mask
    for data in data_load:
        for i, j in data:#每一个data都是一个tuple，i代表test图(anomaly or normal),j 代表对应的mask(anomaly mask or normal mask)
            if i.size(1)==1:
    ...
    if thres_type == 0 :
        ...
        precision, recall, thresholds = precision_recall_curve(masks, scores)#用预测分数score和真实数据做精度结算
...
def Patch_Overlap_Score(threshold, data_load = loader[1:], upsample =1):
    ...
    for n,data in enumerate(data_load):
        total_loss_all = []
        for c,(i, j) in enumerate(data):#c 代表数据列表元组的下标
            if i.size(1)==1:
                i = torch.stack([i,i,i]).squeeze(2).permute(1,0,2,3)
            vector, reconstructions = model(i.cuda())
            pi, mu, sigma = G_estimate(vector)
            ...
            ## Plotting
            if c%5 == 0:
                plot(i,j,score_map[0][0])#i 代表真实图，j代表真实图的mask，score_map[0][0] 代表热力图
            if n == 0:
