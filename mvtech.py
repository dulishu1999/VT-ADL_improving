                ...
                for i in file:
                    if os.path.isdir(os.path.join(root, d, d_in,i)):
                        ...
                        if (data_motive == 'test') :
                            ...
                            elif (use_good == False) and (i == 'ko') and normal != True :
                                tr_img_pth = os.path.join(root, d, d_in,i)
                                images = os.listdir(tr_img_pth)
                                im_pt[tr_img_pth] = images
                                print(f'total {d_in} images of {i} {d} are: {len(images)}')
                            elif (use_good == True) and (i == 'ok') and (normal== True):
                                tr_img_pth = os.path.join(root, d, d_in,i)
                                images = os.listdir(tr_img_pth)
                                im_pt[tr_img_pth] = images
                                print(f'total {d_in} images of {i} {d} are: {len(images)}') 
...
class Mvtec:
    def __init__(self, batch_size,root="F:\\ckj-deeplearning\\BTech_Dataset_transformed", product= 'bottle'):
        self.root = root
    ...
    #           transforms.Normalize((0.1307,), (0.3081,)),
            ...
            #就是在这，对于测试异常数据时的mask是自己手动标注的位置的
            test_anom = tuple(zip(test_anom_image, test_anom_mask))
            #zip 就是将两个list一一对应形成一个大的tuple，比如list1[a1,a2,a3,a4],list2[b1,b2,b3,b4],zip之后就成了tuple[{a1,b1},{a2,b2},{a3,b3},{a4,b4}]
